...
export default function Button({
  ...
  return (
    <button
      ...
      className={`overflow-hidden ${disabled ? "bg-complementary2" : bgColor ? bgColor : "bg-gradient-to-r from-secondary2 to-secondary"} ${bgColor === "bg-danger" && "hover:bg-wait"} ${bgColor === "bg-wait" && "hover:bg-danger"} ${!disabled && "hover:bg-gradient-to-br"} bold animate-fadeIn ${widthFull && "w-full"} max-w-[300px] min-h-[20px] flex justify-center items-center gap-2 shadow-md py-1 px-4 transition-all duration-300 rounded-full`}
      onClick={click && click}
